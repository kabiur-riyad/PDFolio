...
let isDirty = false;
let baselineSnapshot = null; // Snapshot of state to compare against
...
function getStateSnapshot() {
  return JSON.stringify({ userInfo, pages });
}

function hasChangesFromBaseline() {
  if (!baselineSnapshot) return true; // No baseline yet, consider it changed
  const current = getStateSnapshot();
  return current !== baselineSnapshot;
}

function setBaseline() {
  baselineSnapshot = getStateSnapshot();
  setDirty(false);
}
...
function markDirty() {
  // Only mark dirty if there are actual changes from baseline
...
function clearDirty() {
  setDirty(false);
  // Also update baseline to current state when explicitly clearing dirty
  baselineSnapshot = getStateSnapshot();
}

// Warn before closing if there are unsaved changes
window.onbeforeunload = function(e) {
  console.log('beforeunload triggered, isDirty:', isDirty);
  if (isDirty) {
...
function renderPagesList() {
  ...
  // Track current series context for visual grouping
  let currentSeriesTitle = null;

  pages.forEach((p, i) => {
    // Check if this is a series cover or first image of a new series
    if (p.type === 'series-cover') {
      currentSeriesTitle = p.seriesTitle;
    } else if (p.type === 'series-image') {
      if (p.seriesTitle !== currentSeriesTitle) {
        // This image belongs to a different series (orphaned or out of order)
        currentSeriesTitle = p.seriesTitle;
      }
    } else {
      // Non-series page resets context
      currentSeriesTitle = null;
    }

    const isSeriesItem = p.type === 'series-cover' || p.type === 'series-image';
    const isSeriesImage = p.type === 'series-image';

    const item = document.createElement('div');
    item.className = 'page-item' + (isSeriesImage ? ' series-image-item' : '');

    // Add visual grouping styles for series items
    if (isSeriesItem) {
      item.style.position = 'relative';
    }
    if (isSeriesImage) {
      item.style.paddingLeft = '20px';
      item.style.borderLeft = '2px solid var(--accent, #4a90d9)';
      item.style.marginLeft = '8px';
      item.style.backgroundColor = 'rgba(74, 144, 217, 0.05)';
    }

    // Add series connector line for images
    if (isSeriesImage) {
      ...
      connector.style.borderLeft = '2px dashed var(--accent, #4a90d9)';
      connector.style.borderBottom = '2px dashed var(--accent, #4a90d9)';
      connector.style.borderBottomLeftRadius = '6px';
    ...
    if (p.image) {
      ...
      thumb.textContent = (p.type === 'cover' ? 'C' : p.type === 'series-cover' ? 'S' : 'P');
    ...
    // Show series title badge for series pages
    const seriesId = p.seriesTitle || (p.seriesObj && p.seriesObj.title);
    const seriesBadge = isSeriesItem && seriesId ? `<span style="display:inline-block;background:var(--accent,#4a90d9);color:#fff;padding:1px 6px;border-radius:10px;font-size:10px;margin-right:6px">${escapeHtml(seriesId)}</span>` : '';
    meta.innerHTML = `<div style="font-weight:600">${seriesBadge}${escapeHtml(itemTitle)}</div>`;
    ...
    del.onclick = () => {
      // Find current index by reference since array may have changed
      const currentIdx = pages.findIndex(pg => pg === p);
    ...
    item.addEventListener('dragstart', (e) => {
      ...
      const seriesId = p.seriesTitle || (p.seriesObj && p.seriesObj.title);
      if (p.type === 'series-cover' && seriesId) {
        const seriesPages = pages.filter(pg => {
          const pgSeriesId = pg.seriesTitle || (pg.seriesObj && pg.seriesObj.title);
          return pgSeriesId === seriesId && (pg.type === 'series-cover' || pg.type === 'series-image');
        });
        e.dataTransfer.setData('seriesPages', JSON.stringify(seriesPages.map(pg => pages.indexOf(pg))));
        // Add visual indication to all series items
        document.querySelectorAll('.page-item').forEach((el, idx) => {
          const page = pages[idx];
          if (page) {
            const pageSeriesId = page.seriesTitle || (page.seriesObj && page.seriesObj.title);
            if (pageSeriesId === seriesId && (page.type === 'series-cover' || page.type === 'series-image')) {
              el.classList.add('series-dragging');
            }
          }
        });
      }
    ...
    item.addEventListener('dragover', (e) => {
      ...
      // Check if we're dragging a series
      const seriesData = e.dataTransfer.getData('seriesPages');
      if (seriesData) {
        // When dragging a series, don't allow dropping onto items within the same series
        const seriesIndices = JSON.parse(seriesData);
        if (!seriesIndices.includes(targetIndex)) {
    ...
    item.addEventListener('drop', (e) => {
      ...
      const fromIndex = parseInt(draggingItem.dataset.index, 10);
      const toIndex = parseInt(item.dataset.index, 10);

      // Check if we're moving a series
      const seriesData = e.dataTransfer.getData('seriesPages');
      if (seriesData && pages[fromIndex] && pages[fromIndex].type === 'series-cover') {
        const seriesIndices = JSON.parse(seriesData).map(Number).sort((a, b) => a - b);
        // Don't drop onto an item within the same series
        if (seriesIndices.includes(toIndex)) {
          item.classList.remove('drag-over');
          return;
        }
        moveSeries(seriesIndices, toIndex);
...
function moveSeries(seriesIndices, targetIndex) {
  // seriesIndices are sorted ascending [coverIndex, image1Index, image2Index, ...]
  // We need to extract all series pages and insert them as a block at targetIndex

  // Get the pages in their current order
  const seriesPages = seriesIndices.map(i => pages[i]);

  // Remove pages from their original positions (in reverse order to maintain indices)
  const sortedIndices = [...seriesIndices].sort((a, b) => b - a);
  sortedIndices.forEach(i => pages.splice(i, 1));

  // Calculate new insertion index after removals
  // If target was after any removed indices, adjust accordingly
  ...
  // Insert series pages as a block at the adjusted target position
  seriesPages.forEach((page, i) => {
    pages.splice(newTargetIndex + i, 0, page);
  });
...
function deletePage(index) {
  ...
  if (!page) {
    console.warn('deletePage called with invalid index:', index, 'pages.length:', pages.length);
    return;
  }

  // Get series identifier from either seriesTitle or seriesObj.title
  const seriesId = page.seriesTitle || (page.seriesObj && page.seriesObj.title);

  // If deleting a series cover, also delete all related series images
  if (page.type === 'series-cover' && seriesId) {
    const relatedCount = pages.filter(p =>
      p && p.type === 'series-image' &&
      (p.seriesTitle === seriesId || (p.seriesObj && p.seriesObj.title === seriesId))
    ).length;
    const confirmMsg = relatedCount > 0
      ? `Delete "${page.data.title}" and its ${relatedCount} image(s)?`
      : `Delete "${page.data.title}"?`;
    if (!confirm(confirmMsg)) return;

    // Find all indices to delete (cover + all related series images)
    const indicesToDelete = [];
    pages.forEach((p, i) => {
      if (!p) return;
      const pSeriesId = p.seriesTitle || (p.seriesObj && p.seriesObj.title);
      if (pSeriesId === seriesId && (p.type === 'series-cover' || p.type === 'series-image')) {
        indicesToDelete.push(i);
      }
    });

    // Sort indices in descending order so we can delete without affecting other indices
    indicesToDelete.sort((a, b) => b - a);
    indicesToDelete.forEach(i => {
      if (pages[i]) pages.splice(i, 1);
    });
  } else {
...
document.getElementById('projectForm').addEventListener('submit', (e) => {
  ...
  const imageTitlePattern = document.getElementById('projectImageTitlePattern').value || 'Image {n}';
  pages.push({ type: 'series-cover', data: { title, year, desc, total: numImages }, image: null, seriesTitle: title });
  for (let i = 0; i < numImages; i++) {
    const imageTitle = imageTitlePattern.replace('{n}', i + 1);
    pages.push({ type: 'series-image', data: { title: imageTitle, desc: '' }, image: null, seriesTitle: title, seriesTotal: numImages });
  }
...
if (newPortfolioBtn) newPortfolioBtn.addEventListener('click', async () => {
  try {
    ...
    // Set baseline for fresh portfolio - not considered "dirty" until changes are made
    setBaseline();
  } catch {}
});
